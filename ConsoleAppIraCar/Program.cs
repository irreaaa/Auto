...
class SportCar : Auto
{
    ...
    protected internal SportCar(string autoNumber, double currentV, double vMax, double rashod, double distance) : base(autoNumber, currentV, vMax, rashod, distance)
    {
        AutoNumber = autoNumber;
        CurrentV = currentV;
        VMax = vMax;
    ...
    protected internal override bool Doedet(double km)
    {
        double neededFuel = Rashod * km;
        return CurrentV >= neededFuel;
    }

    protected internal override double Proekhat(double km)
    {
        if (!Doedet(km))
        {
            Console.WriteLine("Недостаточно топлива для преодоления указанного расстояния.");
    ...
    protected internal override void Perezaryad(double litres)
    {
        if (litres <= 0)
        {
            Console.WriteLine("Объем топлива не может быть отрицательным.");
    ...
    protected internal override void Itog(double distance)
    {
        if (Doedet(distance))
        {
